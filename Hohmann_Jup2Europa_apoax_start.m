%Calcolo del delta v necessario per effettuare una manovra di Hohmann tra 
%orbita ellittica intorno a Giove ed Europa. Non sarà una
%manovra di Hohmann classica perchè devo considerare che la traiettoria di
%partenza è ellittica. Inoltre calcolo il delta v necessario per entrare in
%orbita attorno ad Europa, mediante iperbole di cattura.

function [delta_v1, delta_t_H_hours, e_hyp, a_hyp, a_H, e_H, delta_v2, delta_v_tot] = Hohmann_Jup2Europa_apoax_start(r_periax, e_init)
...
a_H = (r_1 + r_2)/2;                        %coefficiente a dell'ellisse di Hohmann
e_H = (r_2 - r_1)/(r_2 + r_1);              %eccentricità ellissi Hohmann

v_init = sqrt(2*mu_j/r_1 - mu_j/a_1);       %velocità all'apogeo dell'orbita di parcheggio intorno a Giove

v_trans_p = sqrt(2*mu_j/r_1 - mu_j/a_H);    %velocità al perigeo dell'orbita di Hohmann
...
delta_t_H = pi*sqrt((r_1+r_2)^3/(8*mu_j));  %tempo per la manovra di Hohmann secondi
delta_t_H_hours = delta_t_H/60;             %tempo Hohmann in ore
