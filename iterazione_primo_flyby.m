...
% da far coincidere le velocità relative di entrata e uscita, minimizzando
...
%Valutazione della velocità relativa post Flyby rispetto a Marte
...
%     v2 = [v2(1), v2(2)];
    ...
    %Valutazione velocità relativa di uscita dal Flyby rispetto a Marte
    ...
    if diff<toll 
        %FLYBY
        [deltav, ~, ~, ~, ~, r_p] = flyby ( v2, v_mars, v_dep, pl_mu(4));
        
        %Se la manovra di Flyby è credibile calcolo il deltav di uscita
        %dalla Terra
        if r_p > 100
            %MANOVRA DI USCITA
            [~, dvu, ~, ~] = escape_hyp (v1, 200, t0);
            
            %Aggiornamento grandezze variabili se la manovra di uscita è più
            %vantaggiosa
                t = [t, t0];
                dv = [dv, deltav];
                r = [r, r_p];
